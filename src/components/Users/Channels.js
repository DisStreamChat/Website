    ...
    return (
        <div className="channel-item">
            ...
            <div className="channel-info">
                ...
                <button disabled={!props.isMember} className="to-dashboard dashboard-button">{props.isMember ? <Link className="dashboard-link" to={`/dashboard/${props.uid}`} >{!props.moderator ? "Go To Dashboard" : "Go To ModView"}</Link> : <>This channel doesn't use DisTwitchChat</>}</button>
            </div>
        </div>
    )
}
...
const Channels = () => {
    ...
    useEffect(() => {
        setMyChannel({ name: currentUser?.name, isMember: true, profilePicture: currentUser?.profilePicture, uid: firebase.auth.currentUser.uid })
    }, [currentUser])
