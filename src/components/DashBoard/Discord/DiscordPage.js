...
const DiscordPage = memo(({ history, match }) => {
	...
	const { userDiscordInfo, userConnectedGuildInfo, setUserConnectedGuildInfo } = useContext(
		DiscordContext
	);
	...
	useEffect(() => {
		(async () => {
			...
			if (guild) {
				...
				const userData = (
					await firebase.db.collection("Streamers").doc(userId).get()
				).data();
	...
	useEffect(() => {
		(async () => {
			...
			setRefreshed(true);
			const response = await fetch(
				`${process.env.REACT_APP_API_URL}/discord/token/refresh?token=${refreshToken}&id=${userId}&otc=${otc}`
			);
			...
			const docRef = firebase.db
				...
				.doc("data");
			try {
				await docRef.update(json.userData);
			} catch (err) {
				docRef.set(json.userData);
			}
		})();
	...
	return (
		<div>
			...
			<h3>
				...
				client/overlay during stream. You can only connect one server at a time but you can
				connect as many channels in that server
			</h3>
			<hr />
					...
					<>
						...
						<div className="discord-header">
							<StyledSelect
								...
								options={userDiscordInfo?.guilds
								...
								!userConnectedGuildInfo.isMember ? (
									<div className="not-member">
										<span className="error-color">
											DisStreamBot is not a member of this server
										</span>
										<A
